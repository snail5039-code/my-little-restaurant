...
- 카카오맵 도메인 등록 + 카카오 로그인 KOE205(동의항목) 문제 — 사용자가 카카오 디벨로퍼스/Supabase 콘솔에서 직접 해결 완료
- UI 전면 리디자인 (타베로그 참고)
  - 색/배경: 따뜻한 오프화이트 배경 톤, 한국어 폰트 폴백 스택, `<html lang="ko">`
  - 사이드바: 모바일에서 상단바 + 슬라이드 드로어로 전환하는 반응형 레이아웃
  - 맛집 카드: 평점을 진한 주황 숫자 배지로 강조, 호버 시 살짝 뜨는 효과
  - 상세 페이지: 이름 옆 큰 평점 배지 + 방문/좋아요/혼밥난이도 칩
  - 로그인 가드를 "비활성 버튼" 대신 `LoginRequiredModal` 팝업으로 교체 — 버튼은 항상 정상으로 보이고 클릭 시 로그인 안내 모달이 뜸
  - 전 페이지 여백을 `p-4`(모바일)/`md:p-8`(데스크톱) 반응형으로 통일
